...
Constante real en `BackEnd/services/common/claude.service.js` (`CATEGORIAS_PERMITIDAS`):
...
1. **`BackEnd/config/claude.js`** — inicializa el cliente `@anthropic-ai/sdk` con `process.env.ANTHROPIC_API_KEY`.
2. **`BackEnd/services/common/claude.service.js`** — prompts de sistema, `CATEGORIAS_PERMITIDAS`, FAQs,
   lógica de enrutamiento por rol/ruta, formateo de CV, y adaptación del historial.
3. **`BackEnd/controllers/common/claude.controller.js`** — expone `chatSoporte` (público, valida
