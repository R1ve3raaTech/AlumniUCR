...
export async function editarBlog(token, id, payload) {
  return apiFetch(`/comunidad/blogs/${id}`, { method: 'PUT', token, body: payload });
}
export async function eliminarBlog(token, id) {
  return apiFetch(`/comunidad/blogs/${id}`, { method: 'DELETE', token });
}
