...
import { obtenerDirectorio } from '../perfil/perfilExalumno';
...
/**
 * Puente entre "Sugeridos" (perfiles del directorio, puntuados en el cliente,
 * sin match real todavía) y el motor RF-06 del BE. Busca si ya existe un match
 * con ese exalumno en `misMatchesActuales`; si no existe, lo genera y vuelve a
 * consultar; si ya fue contactado antes, no reintenta (el BE solo permite
 * contactar matches en estado "sugerido").
 */
export async function conectarConExalumno(token, idExalumno, misMatchesActuales) {
  let match = (misMatchesActuales || []).find((m) => m.usuarios?.id === idExalumno);

  if (!match) {
    await generarMatches(token);
    const actualizados = await obtenerMisMatches(token);
    match = actualizados.find((m) => m.usuarios?.id === idExalumno);
  }

  if (!match) throw new Error('No se pudo generar la conexión con este exalumno.');
  if (match.estado !== 'sugerido') return match;

  return contactarMatch(token, match.id);
}
