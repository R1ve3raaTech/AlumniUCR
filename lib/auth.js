...
/**
 * Registro directo de estudiante (@ucr.ac.cr + contraseña). La cuenta nace
 * activa (sin aprobación) y la respuesta incluye una sesión para el auto-login.
 */
export async function registrarEstudiante(payload) {
  return apiFetch('/auth/register/estudiante', {
    method: 'POST',
    body: payload,
  });
}
...
/** Etapa 1: solicita el envío del código de verificación al correo. */
...
/**
 * Etapa 1.5: verifica el código de 6 dígitos del correo.
 * Devuelve { uid, token } para la página /restablecer.
 */
export async function verificarCodigoRecuperacion(correo, codigo) {
  const res = await apiFetch('/auth/recuperar/verificar', {
    method: 'POST',
    body: { correo, codigo },
  });
