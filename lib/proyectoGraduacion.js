...
/** Relación área temática <-> proyecto ya guardada, o null si aún no existe. */
export async function obtenerAreaTematicaDelProyecto(token, idProyecto) {
...
/** Crea o actualiza la relación de área temática del proyecto. */
export async function guardarAreaTematica(token, idProyecto, idAreaTematica, relacionExistente) {
  if (relacionExistente) {
    return apiFetch(`/areas-interes-proyectos/${relacionExistente.id}`, {
      method: 'PUT',
      token,
      body: { id_area_tematica: idAreaTematica },
    });
  }
